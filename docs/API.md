...
| POST | `/export/import/:entity` | Multipart upload (`file`: .csv/.xlsx/.json) for `vendors`, `printers`, `departments`. Duplicates are validated and reported, never overwritten. **Printers** accept the columns `Department`, `IP Address`, `Model`, `Serial Number`, `Status`, `Ownership Status`, `Toner Model`, `Waste Toner Model`, `Item Type`, `Path` (header matching is case/spacing-insensitive). Only `Model` is required; unknown departments are auto-created; `Toner Model` may hold several toners (one per line, `"<Colour> <ModelCode>"`) which become individual consumables; duplicates are detected by serial number, IP address or asset number |
